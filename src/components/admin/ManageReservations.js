...
const initialFilters = { page: 1, limit: 20, search: "", scope: "today", status: "all", partySize: "", dateFrom: "", dateTo: "" };
...
const ManageReservations = () => {
  ...
  useEffect(() => {
    ...
    socket.on("reservation:new", incoming); socket.on("reservation:status_changed", queue); socket.on("connect", reconnect);
    return () => { clearTimeout(socketTimer.current); socket.off("reservation:new", incoming); socket.off("reservation:status_changed", queue); socket.off("connect", reconnect); };
  }, [load, selectedId]);
